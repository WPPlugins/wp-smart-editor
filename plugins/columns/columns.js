(function () {
    parent.jQuery('#TB_window,#TB_iframeContent').width(960);
    parent.jQuery('#TB_window').height(565);
        ...
        if (typeof (editor) !== 'undefined') {
            ...
            if (par) {
                ...
                for (var i = 0; i < csColumns.length; i++) {
                    ...
                    var csHTML = csColumn.getElementsByClassName('csColumn_text')[0].innerHTML;
                    var posX0 = editor.dom.getAttrib(csColumn, 'data-posx0');
                    var posX1 = editor.dom.getAttrib(csColumn, 'data-posx1');
                    var col_pad = editor.dom.getAttrib(csColumn, 'data-padding');
                    var colWidth = (posX1 - posX0) / 100 * defaultMax;
...
function getColumnsHtml() {
    var $ = jQuery;
    var columnsHtml = '';
    var defaultMax = $('.column-sliders').first().width();
    var imgGap = '<img src="'+ myCol.url +'img/1x1-pixel.png" />';
    var lastPoint = 0;
    var totalWidth = 0;
    var col_padding = $('#col_padding').val();
    if ($('.columns_preview').length > 0) {
        columnsHtml += '<div class="csRow">';
        $('.columns_preview').each(function (index, e) {
            ...
            columnsHtml += '<div data-posX0="'+ posX0 +'" data-posX1="'+ posX1 +'" data-padding="'+col_padding+'" class="csColumn" style="margin: 0; padding: 0; float: left; width:'+ colWidth+'%;" ><div style="padding: '+col_padding+'px;"><div class="preview_text csColumn_text">' + editorContent +'</div></div></div>';
        });
